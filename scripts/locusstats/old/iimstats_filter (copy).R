...
maxmiss <- as.integer(args[3]) # Max missing data (Ns) in %
mindist <- as.integer(args[4])
dxy_loci <- as.character(args[5])
...
################################################################################
#lstats_backup <- lstats # lstats <- lstats_backup
nrow_org <- nrow(lstats)
...
filter_proxim <- function(my.lstats, comp, my.mindist = mindist) {
  ...
  #quantile(my.lstats$distToNext, na.rm = TRUE)
