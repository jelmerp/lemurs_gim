...
do
	#LINE=1
	#FASTA_IN=/datacommons/yoderlab/users/jelmer/proj/hybridzone/seqdata/stacks//hzproj//grimurche.og//fasta/hzproj.grimurche.og.merged.filteredloci.fasta
	#LOCUSLIST=/datacommons/yoderlab/users/jelmer/proj/iim/locusstats/grimurche.iim1.locuslist.txt
	
	LOCUS=$(head -n $LINE $LOCUSLIST | tail -n 1)
	echo -e "#### iimstats_splitfasta.sh: Line: $LINE  Locus: $LOCUS"
	
	## Get single-locus, multi-ind fasta:
	grep -A 1 ">${LOCUS}_" $FASTA_IN | sed -E 's/L[0-9]+_([a-z].*_A[0-1])_.*/\1/' | grep -v "\-\-" > $DIR_LOCUSFASTA_OG/$LOCUS.fa
	...
	egrep -A 1 "${IND1}_A0|${IND2}_A0" $DIR_LOCUSFASTA_OG/$LOCUS.fa | grep -v "\-\-" > $DIR_LOCUSFASTA_BW/$LOCUS.fa
done
