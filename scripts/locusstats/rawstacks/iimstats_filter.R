...
setID <- args[1]
dir_locusstats <- args[2]
maxmiss <- as.integer(args[3]) # Max missing data (Ns) in % [5]
mindist <- as.integer(args[4]) # Min distance in bp between loci [10,000]
dxy_loci <- as.character(args[5]) # "include" or "exclude"
outfile_suffix <- args[6]
...
cat("#### iimstats_filter.R: Dxy loci:", dxy_loci, "\n")
...
if(dxy_loci == "include") {
  cat("#### Including Dxy loci...\n")
  lstats <- lstats_raw %>% filter(!is.na(start))
} else {
  cat("#### Excluding Dxy loci...\n")
  lstats <- lstats_raw %>% filter(is.na(start))
}
...
################################################################################
##### FILTER BECAUSE DXY STATS ARE MISSING #####
################################################################################
#nrow_org <- nrow(lstats)
#lstats <- lstats %>% filter(!is.na(dxy))
#nloci_rm_noDxy <- nrow_org - nrow(lstats)
#cat("#### Nr loci removed - no dxy data:", nloci_rm_noDxy, "\n")
...
################################################################################
#locus    scaffold    start length nvar_w1 nvar_w2 nvar_bw nvar_og   dxy pmiss_w1 pmiss_w2
#1 370611 NC_033692.1 82531843    789      14      13       1    4.95  7.10     0.00     0.13
#2 371334 NC_033692.1 91799674    755      14      14      14    9.44 16.44     0.26     0.00

maxvar <- 10
lstats_fake <- lstats %>% filter(nvar_w1 <= maxvar, nvar_w2 <= maxvar)

nloci_rm_nsnp1 <- nrow(lstats) - nrow(lstats_fake)
cat("#### Nr loci THAT WOULD BE removed - too many SNPS:", nloci_rm_nsnp1, "\n")


################################################################################
##### FILTER: NR OF WITHIN-SP SNPS SHOULD BE SMALLER THAN WITH OUTGROUP #####
################################################################################
lstats_fake <- lstats %>% filter(nvar_w1 <= dxy, nvar_w2 <= dxy)

nloci_rm_nsnp2 <- nrow(lstats) - nrow(lstats_fake)
cat("#### Nr loci THAT WOULD BE removed - more within than outgroup SNPS:", nloci_rm_nsnp2, "\n")
